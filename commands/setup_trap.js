...
module.exports = {
    name: "setup_trap",
    description: "Membuat channel jebakan untuk diban member yang melakuakn advertising",
    options: [
        {
            name: "channel",
    ...
    async execute(interaction) {
        ...
        if (interaction.memberPermissions?.has(PermissionsBitField.Flags.Administrator)) {
            try {
                const isChanelExist = await TrapChannel.findOne({ guildId: interaction.guildId });

                if (isChanelExist) {
                    await TrapChannel.updateOne(
                    ...
                    const newTrapChannel = new TrapChannel({
                        guildId: interaction.guildId,
                        channelId: targetChannel.id,
                    });
                    await newTrapChannel.save();
